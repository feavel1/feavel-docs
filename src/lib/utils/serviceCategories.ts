...
export async function getServiceCategories(supabase: SupabaseClient) {
	const { data, error } = await supabase.from('services_category').select('id, category_name').order('category_name');
...
export async function addCategoriesToService(supabase: SupabaseClient, serviceId: number, categoryNames: string[]) {
	try {
		...
		for (const categoryName of categoryNames) {
			...
			await supabase.from('services_category').upsert({ category_name: categoryName }, {
...
export async function removeCategoriesFromService(supabase: SupabaseClient, serviceId: number) {
	const { error } = await supabase.from('services_category_rel').delete().eq('service_id', serviceId);
	
	return { error };
}

export async function updateServiceCategories(supabase: SupabaseClient, serviceId: number, categoryNames: string[]) {
	try {
		...
		const existingCategoryNames = existingRelations?.map(rel => (rel as any).services_category?.category_name).filter(Boolean) || [];
		
		// Find categories to add (in new list but not in existing)
		const categoriesToAdd = categoryNames.filter(categoryName => !existingCategoryNames.includes(categoryName));
		
		// Find categories to remove (in existing but not in new list)
		const categoriesToRemove = existingCategoryNames.filter(categoryName => !categoryNames.includes(categoryName));
